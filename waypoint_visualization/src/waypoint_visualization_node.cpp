...
private:
    ...
    void SetGoalRadiusCallback(const std_msgs::Float64::ConstPtr &);
...
Node::Node() : nh(), private_nh("~"), interactive_server("waypoint_visualization_node")
{
    ...
    pose_array_publisher = nh.advertise<geometry_msgs::PoseArray>(
        param.pose_array_topic,
        param.publish_queue_size,
        param.latch);
    update_waypoint_publisher = nh.advertise<waypoint_manager_msgs::WaypointStamped>(
        param.update_waypoint_topic,
        param.publish_queue_size,
        param.latch);
    erase_waypoint_publisher = nh.advertise<std_msgs::String>(
        param.erase_waypoint_topic,
        param.publish_queue_size,
        param.latch);
    append_route_publisher = nh.advertise<std_msgs::String>(
        param.append_route_topic,
        param.publish_queue_size,
        param.latch);
    erase_route_publisher = nh.advertise<std_msgs::String>(
        param.erase_route_topic,
        param.publish_queue_size,
        param.latch);
    insert_route_publisher = nh.advertise<std_msgs::String>(
        param.insert_route_topic,
        param.publish_queue_size,
        param.latch);
    route_path_publisher = nh.advertise<nav_msgs::Path>(
        param.route_path_topic,
        param.publish_queue_size,
        true);

    route_subscriber = nh.subscribe<waypoint_manager_msgs::Route>(
        param.route_topic,
        param.subscribe_queue_size,
        &Node::routeCallback,
        this);
    waypoints_subscriber = nh.subscribe<waypoint_manager_msgs::Waypoints>(
        param.waypoints_topic,
        param.subscribe_queue_size,
        &Node::waypointsCallback,
        this);

    goal_radius_subscriber = nh.subscribe<std_msgs::Float64>(
        "set_goal_radius",
        1,
        &Node::SetGoalRadiusCallback,
        this);
    ...
    auto white_line_menu_id = menu_handler.insert(
        ...
        "White line point",
        std::bind(
            &Node::whitelinePointFeedback,
            this,
            std::placeholders::_1));
    menu_handler.setCheckState(white_line_menu_id, interactive_markers::MenuHandler::UNCHECKED);
    ...
    auto switch_segmentation_menu_id = menu_handler.insert(
        properties_menu_id,
        "Switch segmentation");
    auto switch_segmentation_child_menu_id1 = menu_handler.insert(
        switch_segmentation_menu_id,
        "ON",
        std::bind(
            &Node::switchsegmentationFeedback1,
            this,
            std::placeholders::_1));
    menu_handler.setCheckState(switch_segmentation_child_menu_id1, interactive_markers::MenuHandler::UNCHECKED);

    auto switch_segmentation_child_menu_id2 = menu_handler.insert(
        switch_segmentation_menu_id,
        "OFF",
        std::bind(
            &Node::switchsegmentationFeedback2,
            this,
            std::placeholders::_1));
    menu_handler.setCheckState(switch_segmentation_child_menu_id2, interactive_markers::MenuHandler::UNCHECKED);

    auto goal_radius_menu_id = menu_handler.insert(
        properties_menu_id,
        "goal radius");
    auto goal_radius_child_menu_id1 = menu_handler.insert(
        goal_radius_menu_id,
        "0.25",
        std::bind(
            &Node::goalradiusFeedback1,
            this,
            std::placeholders::_1));
    menu_handler.setCheckState(goal_radius_child_menu_id1, interactive_markers::MenuHandler::UNCHECKED);

    auto goal_radius_child_menu_id2 = menu_handler.insert(
        goal_radius_menu_id,
        "0.5",
        std::bind(
            &Node::goalradiusFeedback2,
            this,
            std::placeholders::_1));
    menu_handler.setCheckState(goal_radius_child_menu_id2, interactive_markers::MenuHandler::UNCHECKED);

    auto goal_radius_child_menu_id3 = menu_handler.insert(
        goal_radius_menu_id,
        "0.75",
        std::bind(
            &Node::goalradiusFeedback3,
            this,
            std::placeholders::_1));
    menu_handler.setCheckState(goal_radius_child_menu_id3, interactive_markers::MenuHandler::UNCHECKED);
}
...
void Node::waypointsCallback(const waypoint_manager_msgs::Waypoints::ConstPtr &msg)
{
    ...
    for (const auto &wp : msg->waypoints)
    {
        ...
        for (const auto &p : wp.properties)
        {
            ...
            if (p.name == "switch_segmentation_ON" && p.data == "true")
            {
...
void Node::whitelinePointFeedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ROS_INFO("Called whitelinePointFeedback %s", feedback->marker_name.c_str());
...
{
    ROS_INFO("Called switchsegmentationFeedback1 %s", feedback->marker_name.c_str());
    ...
    if (menu_check_state == interactive_markers::MenuHandler::CHECKED)
    {
        ...
        msg.waypoint.properties.push_back(waypoint_manager_msgs::Property());
        msg.waypoint.properties.back().name = "switch_segmentation_ON";
        msg.waypoint.properties.back().data = "false";
    }
    else if (menu_check_state == interactive_markers::MenuHandler::UNCHECKED)
    {
...
void Node::switchsegmentationFeedback2(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ROS_INFO("Called switchsegmentationFeedback2 %s", feedback->marker_name.c_str());
    ...
    if (menu_check_state == interactive_markers::MenuHandler::CHECKED)
    {
        ...
        msg.waypoint.properties.push_back(waypoint_manager_msgs::Property());
        msg.waypoint.properties.back().name = "switch_segmentation_OFF";
        msg.waypoint.properties.back().data = "false";
    }
    else if (menu_check_state == interactive_markers::MenuHandler::UNCHECKED)
    {
...
void Node::goalradiusFeedback1(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ROS_INFO("Called goalradiusFeedback %s", feedback->marker_name.c_str());
    ...
    else if (menu_check_state == interactive_markers::MenuHandler::UNCHECKED)
    {
        ...
        msg.waypoint.properties.push_back(waypoint_manager_msgs::Property());
        msg.waypoint.properties.back().name = "goal_radius";
        msg.waypoint.properties.back().data = "0.25";
    }
    else
    {
        menu_handler.setCheckState(entry_menu_id, interactive_markers::MenuHandler::UNCHECKED);
...
void Node::goalradiusFeedback2(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ROS_INFO("Called goalradiusFeedback %s", feedback->marker_name.c_str());
    ...
    else if (menu_check_state == interactive_markers::MenuHandler::UNCHECKED)
    {
        ...
        msg.waypoint.properties.push_back(waypoint_manager_msgs::Property());
        msg.waypoint.properties.back().name = "goal_radius";
        msg.waypoint.properties.back().data = "0.5";
    }
    else
    {
        menu_handler.setCheckState(entry_menu_id, interactive_markers::MenuHandler::UNCHECKED);
...
void Node::goalradiusFeedback3(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ROS_INFO("Called goalradiusFeedback %s", feedback->marker_name.c_str());
    ...
    else if (menu_check_state == interactive_markers::MenuHandler::UNCHECKED)
    {
        ...
        msg.waypoint.properties.push_back(waypoint_manager_msgs::Property());
        msg.waypoint.properties.back().name = "goal_radius";
        msg.waypoint.properties.back().data = "0.75";
    }
    else
    {
        menu_handler.setCheckState(entry_menu_id, interactive_markers::MenuHandler::UNCHECKED);
...
void Node::SetGoalRadiusFeedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ...
    else if (menu_check_state == interactive_markers::MenuHandler::UNCHECKED)
    {
        ...
        msg.waypoint.properties.push_back(waypoint_manager_msgs::Property());
        msg.waypoint.properties.back().name = "goal_radius";
        msg.waypoint.properties.back().data = Data_;
    }
    else
    {
        menu_handler.setCheckState(entry_menu_id, interactive_markers::MenuHandler::UNCHECKED);
...
void Node::SetGoalRadiusCallback(const std_msgs::Float64::ConstPtr &msg)
{
    Data = msg->data * 0.01;
    ROS_INFO("set_goal_radius : %f", Data);
}
